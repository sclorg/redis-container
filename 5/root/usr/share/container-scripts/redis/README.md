...
The RHEL image is available in the [Red Hat Container Catalog](https://access.redhat.com/#/registry.access.redhat.com/rhel8-beta/redis-5)
as registry.access.redhat.com/rhel8-beta/redis-5.
...
For this, we will assume that you are using the `rhel8-beta/redis-5` image.
If you want to set only the mandatory environment variables and not store
the database in a host directory, execute the following command:

```
$ docker run -d --name redis_database -p 6379:6379 rhel8-beta/redis-5
```
...
```
$ docker run -d --name redis_database -e REDIS_PASSWORD=strongpassword rhel8-beta/redis-5
```
